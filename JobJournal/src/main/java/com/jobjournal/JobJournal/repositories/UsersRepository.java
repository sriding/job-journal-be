...
public interface UsersRepository extends JpaRepository<Users, Long> {
    ...
    @Query(value = "SELECT u._user_id, u._auth0_id, up._profile_id, up._profile_name, up._user_id_fk_profile, s._setting_id, s._user_id_fk_setting FROM users u INNER JOIN userprofiles up ON up._user_id_fk_profile = u._user_id INNER JOIN setting s ON s._user_id_fk_setting = u._user_id WHERE u._user_id = ?1", nativeQuery = true)
